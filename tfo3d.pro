...
pcl = "Output/Circuit/Loss/"; // For results
...
Group{
  ...
  Cut_Primary    = #{1002002}; // Default: pri-1 sec0-2 sec1-3
  Cut_Secondary0 = #{1002001}; //1


  Surf_In  = Region[{Surf_P_In,  Surf_S0_In}];
  Surf_Out = Region[{Surf_P_Out, Surf_S0_Out}];

  Winding      = Region[{Primary, Secondary0}];
  Skin_Winding = Region[{Skin_Primary, Skin_Secondary0}];
  
  If (_flag_interleave)
	Secondary1 = #SECONDARY1;
	Secondary += Region[{Secondary1}];
	Surf_S1_In = #{IN_SEC1};
	Surf_S1_Out = #{OUT_SEC1};
	Skin_Secondary1 = #{SKIN_SECONDARY1};
    Cut_Secondary1 = #{1002103};
	Surf_In  += Region[{Surf_S1_In}];
    Surf_Out += Region[{Surf_S1_Out}];
    Winding  += Region[{Secondary1}];
    Skin_Winding += Region[{Skin_Secondary1}];
  EndIf
  ...
  If(_winding_model == STRANDED)
    ...
    If(_divJ_zero == DIVJ0_STRONG)
      NbSrc_DomainB = (_flag_interleave)? 3 : 2;
...
Function{
  ...
  DefineConstant[
    ...
    mur_fe0 = { 2000, Min 100, Max 2000, Step 100,
...
/*
  ...
  *///===============================================
  ...
  Rdc_sec = (_flag_interleave)? Rdc_sec0 + Rdc_sec1 : Rdc_sec0;
...
/*     skin_rhor_pri[] = InterpolationLinear[$1]{List[skin_rhor_Circ_pri]} ;
    ...
    skin_rhor_pri[] = InterpolationLinear[$1]{List[skin_rhor_Circ_sec]} ;
    skin_rhoi_pri[] = InterpolationLinear[$1]{List[skin_rhor_Circ_sec]} ;
    prox_nur_pri[]  = InterpolationLinear[$1]{List[prox_nur_Circ_sec]} ;
    prox_nui_pri[]  = InterpolationLinear[$1]{List[prox_nui_Circ_sec]} ;	
...
PostProcessing {
  ...
  If (_flag_circuit_coupling)
     ...
     If (_analysis_type == DYN)
      ...
      { Name P_prox ; Value { // Joule Losses -- proximity loss due to magnetic field (two following formulae are equivalent)
          //Integral { [ 0.5*Re[{d a}*Conj[ Complex[2*Pi*Freq*Im[nu[]],-Re[nu[]]]*{d a}]] ]; In Domain ; Jacobian Vol ; Integration II ; }
          Integral { [ 0.5*SquNorm[{d a}]*2*Pi*Freq*Im[nu[]] ]; In Domain ; Jacobian Vol ; Integration II ; }
        }
      }
	EndIf  
...
/*       Print[ P_skin, OnRegion Rz_pri, Format TimeTable, File > StrCat[Dir,"Rac_pri",ExtGnuplot],
    ...
    If (_analysis_type == DYN)
      ...
      Print[ P_skin, OnRegion Rz_pri, Format TimeTable, File > StrCat[Dir,"SkinLoss",ExtGnuplot],
        SendToServer StrCat[pcl,"0Skin loss in winding [W]"]{0}, Color "LightYellow"];		
      Print[ P_prox[#{Primary}], OnGlobal, Format TimeTable, File > StrCat[Dir,"ProximityLossPri",ExtGnuplot],
        SendToServer StrCat[pcl,"1Proximity loss in pri winding [W]"]{0}, Color "LightYellow"];
      Print[ P_prox[#{Secondary0}], OnGlobal, Format TimeTable, File > StrCat[Dir,"ProximityLossSec",ExtGnuplot],
        SendToServer StrCat[pcl,"2Proximity loss in sec winding [W]"]{0}, Color "LightYellow"];		
      Print[ P_prox[#{Core}], OnGlobal, Format TimeTable, File > StrCat[Dir,"CoreLoss",ExtGnuplot],
        SendToServer StrCat[pcl,"3Core loss [W]"]{0}, Color "LightYellow"];
