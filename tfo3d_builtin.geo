...
llcoil_in() += ll_pri_in;
llcoil_out() += ll_pri_out;
llcoil_in() += ll_sec0_in;
llcoil_out() += ll_sec0_out;

If (_flag_interleave)
  llcoil_in() += ll_sec1_in;
  llcoil_out() += ll_sec1_out;
EndIf
//surf_box() += news; Plane Surface(news) = {llb+2,ll_pri_in,ll_sec0_in,ll_sec1_in};
surf_box() += news; Plane Surface(news) = {llb+2,llcoil_in()};
...
surf_box() += news; Plane Surface(news) = {llb+5,llcoil_out()};
...
air_around = newv; Volume(newv) = {sl_box, sl_skin[], sl_aux};
